...
(function ($) {
			...
			if ($('.product-single').length || $('.main-content').length) {
				setInterval(function () {
					Porto.miniPopup.open({
						name: 'Mobile Speaker',
						nameLink: 'product.html',
						imageSrc: '/public/assets/images/products/small/product-1.jpg',
	...
	Porto.initProductSingle = (function () {
		...
		var initVariation = function (self) {
			self.$selects = self.$wrapper.find('.product-single-filter #size');
		...
		ProductSingle.prototype.variationCheck = function () {
			...
			self.$selects.each(function () {
				return selected_value = this.value || (isAllSelected = false);
			});
			...
			isAllSelected ?
				self.variationMatch(selected_value) :
				self.variationClean();
		}

		ProductSingle.prototype.variationMatch = function (selected_value) {
			var self = this;
			self.$priceWrap.find('span').text(new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(selected_value));
			self.$priceWrap.find('del span').text(selected_value + 10000 + 'VNĐ');
